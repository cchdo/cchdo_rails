...
Grat.prototype.redraw = function(force) {
  ...
  for (var lng = llng-dlng; lng < ulng+dlng; lng += dlng) {
    ...
    this.labels.push(this.make_label(px.x+6, px.y, this.precisify((lng > 180) ? lng - 360 : lng, dlng)));
  }
  ...
  for (var lat = llat-dlat; lat < ulat+dlat; lat += dlat) {
    ...
    this.labels.push(this.make_label(px.x+6, px.y, this.precisify(lat, dlat)));
  }

  for (var i=0; i<this.labels.length; i++) {
    pane.appendChild(this.labels[i]);
  }

  var crit_zoom = zoom >= this.zoom_lim; /* point where drawing vast numbers of overlays is slow. */
  if (zoom != this.last_zoom ||
      maptype != this.last_maptype ||
      size.width != this.last_size.width ||
      size.height != this.last_size.height ||
      crit_zoom) {
    this.remove_lines();
