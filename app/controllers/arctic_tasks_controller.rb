require 'csv'
class ArcticTasksController < ApplicationController
  layout "staff"
  ...
  def index
     ...
     @user = @user.username
     @assignments = ArcticAssignment.find(:all,:order=>["priority"])
     #assignment_list
  ...
  def search_assignments
     ...
     @dir = []
     @text
     @hide_coworkers_checked = false
     @hide_completed_checked = false
     @user = User.find(session[:user])  ## Do these lines need to
     @user = @user.username             ## be here?
     @query = params[:query]
     ...
     if(params[:commit] =~ /ave/)# or params[:commit] =~ /update/)
        @assignments = []
        # Save all changes
        ArcticAssignment.update(params[:assignment].keys, params[:assignment].values)
        #if params[:commit] =~ /update/
        #  for param in params.keys
        #       if param =~ /(snooze)/
        #          (trash,@assignment_id) = param.split('_')
       #          @assignment = ArcticAssignment.find(@assignment_id)
       #          @new_deadline = nil
       #          #@assignment_deadline = ArcticAssignment.find_by_sql("SELECT UNIX_TIMESTAMP(deadline) from assignments where id = '#{assignment_id}'")
       #          case params[:"#{param}"]
       ...
       # end
        if @query
           if @query.eql?("all")
              if completed_string =~ /(priority)|(cchdo_contact)/
                 @assignments = ArcticAssignment.find(:all,:conditions => ["#{completed_string}"],:order=>["#{@sort}"])
              else
                 @assignments = ArcticAssignment.find(:all,:order=>["#{@sort}"])
              end
           else
              for column in ArcticAssignment.columns
                 @names << column.human_name
                 unless (column.name.eql? "history" or column.name.eql? "manager")
                    @results = ArcticAssignment.find(:all ,:conditions => ["`#{column.name}` regexp '#{@query}' #{completed_string}"],:order=>["#{@sort}"])
        ...
        if (params[:query])
           ...
           if @query.eql?("all")
              ...
              end # for column in ArcticAssignment.columns
           end # if @query.eql?("all") .. else .. end
        else
           if completed_string =~ /(priority)|(cchdo_contact)/
              @assignments = ArcticAssignment.find(:all,:conditions => ["`priority` != 6"],:order=>["#{@sort}"])
           else
              @assignments = ArcticAssignment.find(:all,:order=>["#{@sort}"])
           end
  ...
  def manager_v
    ...
    @assignments = ArcticAssignment.find(:all)
  ...
  def csv_dump
     ...
     if @query.eql?("all")
        if completed_string =~ /priority/
           @assignments = ArcticAssignment.find(:all,:conditions => ["#{completed_string}"])
        else
           @assignments = ArcticAssignment.find(:all)
        end
     else
        for column in ArcticAssignment.columns
           ...
           unless (column.name.eql? "history" or column.name.eql? "manager" or column.name.eql? "id" or column.name.eql? "complete")
              @results = ArcticAssignment.find(:all ,:conditions => ["`#{column.name}` regexp '#{@query}' #{completed_string}"])
              if @results.length > @cur_max
                 @cur_max = @results.length
                 @best_result = []
                 @assignments = @results
     ...
     if @assignments
        ...
        CSV::Writer.generate(report, ',') do |csv|
           ...
           @assignments.each do |task|
              csv << [task.ExpoCode, task.project, task.current_status,  task.cchdo_contact, task.data_contact, task.action, task.parameter, task.priority, task.deadline, task.LastChanged, task.notes, task.history]
           end
        end
        report.rewind
  ...
  def create
     @user = User.find(session[:user])
     @user = @user.username
     @assignment = ArcticAssignment.create(params[:assignment])
     @assignments = ArcticAssignment.find(:all,:conditions =>["ExpoCode = '#{@assignment.ExpoCode}'"])
  ...
  def hide_task
    ...
    @assignment_obj = ArcticAssignment.find(@assignment_id)
